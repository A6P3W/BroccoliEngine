...
def TemplateDestination(RelativePath: Path, ProjectName: str) -> Path:
  RenderedPath = Path(*(Part.replace("{PROJECT_NAME}", ProjectName) for Part in RelativePath.parts))
...
def RenderTemplates(
  ...
  TemplateFiles = sorted(PathValue for PathValue in TemplateRoot.rglob("*") if PathValue.is_file())
...
def VerifySubmodule(ProjectRoot: Path, HeadCommit: str) -> None:
  StageOutput = RunGit(ProjectRoot, "ls-files", "--stage", "BroccoliEngine").stdout.strip()
  Fields = StageOutput.split()
  if len(Fields) < 4 or Fields[0] != "160000" or Fields[1] != HeadCommit:
    raise SetupError(
      f"BroccoliEngine was not registered as the expected gitlink: {StageOutput or '<empty>'}"
    )

  StatusOutput = RunGit(ProjectRoot, "submodule", "status", "BroccoliEngine").stdout.rstrip()
...
def ReconfigureInPlace(
  ...
  try:
    ...
    with tempfile.TemporaryDirectory(prefix="broccoli-project-") as TemporaryName:
      ...
      GeneratedPaths = [
        ...
        for Source in sorted(
          PathValue for PathValue in TemporaryRoot.rglob("*") if PathValue.is_file()
        )
    ...
    try:
      ...
      raise SetupError(
        f"Project setup failed during {State.name}: {Error}; rollback failed {RollbackError}"
      ) from Error
    if isinstance(Error, SetupError):
      raise
    raise SetupError(f"Project setup failed during {State.name}: {Error}") from Error
...
def LoadProjectMarker(ProjectRoot: Path) -> dict[str, object]:
  ...
  if not Marker.is_file():
    raise SetupError(
      "This directory is not a generated Broccoli project: .broccoli-project.json was not found."
    )
...
def ValidateResetProject(ProjectRoot: Path, BackupRoot: Path) -> None:
  ...
  TopLevel = RunGit(ProjectRoot, "rev-parse", "--show-toplevel").stdout.strip()
  if Path(TopLevel).resolve() != ProjectRoot.resolve():
    raise SetupError("The current directory must be the generated Git repository root.")
  ...
  ModuleGit = ProjectRoot / ".git" / "modules" / "BroccoliEngine"
  if not EngineGit.is_file() or not ModuleGit.is_dir():
    raise SetupError("BroccoliEngine does not have an absorbed submodule Git directory.")
  ...
  if not ResolvedBackupRoot.parent.is_dir():
    raise SetupError(f"Reset backup parent directory was not found: {ResolvedBackupRoot.parent}")
...
def ParseArguments() -> argparse.Namespace:
  ...
  Mode.add_argument("--output", type=Path, help="Render templates into this directory")
  Mode.add_argument(
    "--in-place",
    action="store_true",
...
def Main() -> int:
  ...
  try:
    if Arguments.reset_project:
      ...
      print("Please specify the path to your toolchain file in CMakeUserPresets.json")
    else:
