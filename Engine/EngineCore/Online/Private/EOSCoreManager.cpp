...
bool EOSCoreManager::Initialize(const FEOSConfig& Config) {
  ...
  M_LOG("EOS_Logging_SetCallback result: {}", SafeEOSString(EOS_EResult_ToString(LogCallbackResult)));
  ...
  if (!IsNullOrEmpty(EncryptionKey) && std::strlen(EncryptionKey) != EOS_PLATFORM_OPTIONS_ENCRYPTIONKEY_LENGTH) {
    M_LOG(
  ...
  bTickLogged = false;
  TickCount = 0;
  M_LOG("EOS_Platform_Create succeeded. PlatformHandle={}", static_cast<const void*>(PlatformHandle));
  return true;
}
