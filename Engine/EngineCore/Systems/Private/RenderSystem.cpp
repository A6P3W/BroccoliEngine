		...
		for (const auto& cmd : commands) {
			...
			std::visit([&](auto&& d) {
				...
				// --- Circle ---
					...
					DrawCircle(static_cast<int>(drawPos.X), static_cast<int>(drawPos.Y), static_cast<int>(drawRadius), d.Color, d.Fill);
				}
				// --- Text ---
				else if constexpr (std::is_same_v<T, TextData>) {
					FVector2D drawPos = (cmd.common.space == RenderSpace::World) ? WorldToScreen(d.Location) : d.Location;
					DrawStringToHandle(static_cast<int>(drawPos.X), static_cast<int>(drawPos.Y), d.Text.c_str(), d.Color, d.Handle);
				}
				// --- Line ---
					...
					DrawLine(static_cast<int>(p1.X), static_cast<int>(p1.Y), static_cast<int>(p2.X), static_cast<int>(p2.Y), d.Color);
				}
				// --- RectGraph ---
					...
					DrawRectGraphF(drawPos.X, drawPos.Y, static_cast<int>(d.SrcLocation.X), static_cast<int>(d.SrcLocation.Y), static_cast<int>(d.SrcSize.X), static_cast<int>(d.SrcSize.Y), d.Handle, TRUE);
				}
