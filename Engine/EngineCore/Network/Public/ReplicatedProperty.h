...
void WriteReplicatedValue(FNetBuffer& OutBuffer, const T& Value) {
  if constexpr (std::is_same_v<T, std::string>) {
    ...
    static_assert(
        ...
        "Replicated properties must be std::string or trivially copyable."
    );
...
class TReplicatedPropertyBase : public IReplicatedProperty {
 public:
  explicit TReplicatedPropertyBase(T* InProperty) : Property(InProperty), CachedValue(*InProperty) {
    assert(InProperty != nullptr);
    static_assert(THasNotEqual<T>::value, "Replicated properties require operator!=.");
  }

  bool HasChanged() const override { return Property && (*Property != CachedValue); }
