...
{
	NetComponentName = std::move(Name);
}
...
void MActorComponent::MarkReplicatedStateDirty()
{
...
bool MActorComponent::InvokeRPCWithPayload(FNetworkRPCId RPCId, ENetRPCType RPCType, ENetPacketReliability Reliability, const FNetBuffer& Payload)
{
	if (RPCId == 0 || !Owner) {
		return false;
	}
