...
def TestStageRuntimeStagesDebugArtifacts(TmpPath: Path) -> None:
    ...
    (EngineDirectory / "Resources" / "engine.txt").write_text(
        "engine", encoding="utf-8"
    )
    (GameDirectory / "Resources" / "level.BLevel.json").write_text(
        "{}", encoding="utf-8"
    )
    (GameDirectory / "Resources-EOS" / "online.txt").write_text(
        "online", encoding="utf-8"
    )
    EngineBinary = TmpPath / "BroccoliEngine.dll"
    EosBinary = TmpPath / "EOSSDK-Win64-Shipping.dll"
...
def TestPackageRuntimeCreatesVerifiedLayout(TmpPath: Path) -> None:
    ...
    (ResourcesDirectory / "Engine" / "engine.txt").write_text(
        "engine", encoding="utf-8"
    )
    (ResourcesDirectory / "Game" / "level.BLevel").write_bytes(b"level")
    (OutputDirectory / "Resources-EOS").mkdir()
    (OutputDirectory / "Resources-EOS" / "online.txt").write_text(
        "online", encoding="utf-8"
    )
