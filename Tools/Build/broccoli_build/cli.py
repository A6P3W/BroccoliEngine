...
def CreateParser() -> argparse.ArgumentParser:
    Parser = argparse.ArgumentParser(
        description="BroccoliEngine build and packaging tools"
    )
    Commands = Parser.add_subparsers(dest="Command", required=True)

    PrepareParser = Commands.add_parser(
        "prepare-output", help="Remove generated runtime output"
    )
    PrepareParser.add_argument("--output-dir", type=PathArgument, required=True)

    StageParser = Commands.add_parser(
        "stage-runtime", help="Stage local runtime dependencies"
    )
    ...
    StageParser.add_argument(
        "--convert-levels-script", type=PathArgument, required=True
    )

    PackageParser = Commands.add_parser(
        "package-runtime", help="Create a distributable package"
    )
    ...
    PackageParser.add_argument(
        "--online-resources-dir", type=PathArgument, required=True
    )
    PackageParser.add_argument(
        "--convert-levels-script", type=PathArgument, required=True
    )
    ...
    VerifyParser = Commands.add_parser(
        "verify-runtime", help="Verify runtime artifacts"
    )
...
def Main() -> int:
    ...
    try:
        if Arguments.Command == "prepare-output":
            ...
            if (
                Arguments.configuration is not None
                and Arguments.configuration.casefold() == "editor"
            ):
                print("Editor configuration: runtime verification skipped.")
            else:
                VerifyRuntime(
                    Arguments.output_dir, Arguments.game_name, Arguments.publish_dir
                )
