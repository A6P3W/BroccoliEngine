...
def CopyFile(Source: Path, DestinationDirectory: Path) -> None:
  ...
  try:
    ...
    raise RuntimeError(f"Failed to copy '{Source}' to '{DestinationDirectory}': {Error}") from Error
...
def CopyDirectory(Source: Path, Destination: Path) -> None:
  ...
  try:
    ...
    raise RuntimeError(f"Failed to copy '{Source}' to '{Destination}': {Error}") from Error


def ConvertLevels(ConvertLevelsScript: Path, ResourcesDirectory: Path, Clean: bool = False) -> None:
