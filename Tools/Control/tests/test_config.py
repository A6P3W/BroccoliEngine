...
from broccoli_control.config import ControlConfig, load_config
...
def test_invalid_timeout_is_rejected(Timeout: float) -> None:
  with pytest.raises(ControlConfigurationError):
    ControlConfig(ConnectTimeoutSeconds=Timeout)
